...
- Satellite rendering now uses delayed two-sample Hermite smoothing with rotating worker traversal to reduce per-object jitter.
...
- Server returns `[tool-history]` `historyMessages`, but Oracle UI does not append them yet.

## Fast Bearings

1. `git rev-parse --short HEAD` and `git status --short`.
2. `npm run dev`.
3. Load `/oracle` in dev (`/satellite-oracle/oracle` in production).
